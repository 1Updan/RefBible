...
import { Bookmark, BookmarkCheck, MessageSquareMore, X, List, XCircle, GripVertical, Share2, Highlighter, Eraser, Sparkles } from 'lucide-react'
      ...
      <div className="fixed bottom-16 left-0 right-0 z-50 flex flex-col items-center px-2 animate-[slideUp_150ms_ease-out] pointer-events-none">
        <div className="w-fit max-w-[96vw] flex items-center gap-0.5 px-2 py-1.5 bg-action-bar shadow-2xl rounded-2xl overflow-x-auto scrollbar-none pointer-events-auto">
                  ...
                  >
                    ...
                    <span className="text-[7px] sm:text-[9px] leading-none">{allBookmarked ? 'Saved' : 'Bookmark'}</span>
                  </button>
          ...
          <span className="w-px h-4 sm:h-5 bg-white/20 shrink-0 mx-0.5" />
                    ...
                    <button
                      ...
                      className="flex flex-col items-center justify-center gap-0.5 px-1.5 py-1 sm:px-2.5 sm:py-2 rounded-lg text-white/60 hover:text-white transition-all duration-150 cursor-pointer min-w-[48px]"
                    >
          ...
          <div className="mt-1.5 flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 bg-action-bar shadow-xl rounded-full pointer-events-auto animate-[fadeIn_100ms_ease-out]">
            {HIGHLIGHT_COLORS.map((c) => (
              <button
                key={c.id}
                type="button"
                onClick={() => onHighlightColorSelect(c.id)}
                className={clsx(
