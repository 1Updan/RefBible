    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        {showSearchResults && (
          <div
            ref={resultsRef}
            className="absolute z-50 left-0 right-12 mt-1 max-h-64 overflow-y-auto rounded-lg bg-surface-elevated border border-border shadow-lg"
          >
            {searching ? (
              ...
              searchResults.map((r) => {
                ...
                return (
                  <div
                    key={r.verse_id}
                    className="w-full text-left px-2.5 py-2 text-xs hover:bg-surface-hover transition-colors duration-100 border-b border-border last:border-b-0 flex items-start gap-1.5"
                  >
                    <button
                      type="button"
                      onClick={() => navigateTo(bookId, r.chapter_num, r.verse_id)}
                    ...
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); handleAdd(r.verse_id) }}
                      disabled={adding}
                      className="flex-shrink-0 mt-0.5 p-1 rounded text-text-tertiary hover:text-accent hover:bg-surface transition-colors duration-150 cursor-pointer disabled:opacity-40"
                      aria-label={`Add ${book?.name ?? ''} ${r.chapter_num}:${r.verse_num} as cross-reference`}
